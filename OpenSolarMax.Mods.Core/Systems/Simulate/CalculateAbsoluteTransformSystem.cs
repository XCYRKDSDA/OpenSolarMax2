...
public sealed partial class CalculateAbsoluteTransformSystem(World world) : ICalcSystem
{
    private static void RecursivelyUpdateAbsoluteTransform(Entity entity)
    {
        ...
        foreach (
            var (relationship, record) in entity
        ...
        )
        {
            ...
            record.Child.Get<AbsoluteTransform>().TransformToRoot =
                transformToParent * parentTransformToRoot;
        }

        // 递归考察子实体
        foreach (
            var (_, record) in entity
                .Get<TreeRelationship<RelativeTransform>.AsParent>()
                .Relationships
        )
    ...
    private static void UpdateFromRoot(
        Entity root,
