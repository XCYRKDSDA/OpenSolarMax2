...
public record ConceptInfo
{
    ...
    private ConceptInfo(
        string name,
        IReadOnlyList<Type> definitionTypes,
    ...
    public static ConceptInfo Define(
        string name,
        Type definitionType,
        Type? descriptionType,
        Type? applierType
    ...
    {
        var expectedApplierInterface = descriptionType is null
            ? typeof(IApplier)
            : typeof(IApplier<>).MakeGenericType(descriptionType);
        if (
            applierType is not null
            && !applierType.GetInterfaces().Contains(expectedApplierInterface)
        )
        ...
        return new ConceptInfo(
            ...
            [definitionType],
            descriptionType,
            applierType is null ? [] : [applierType]
        );
    }
    ...
    public ConceptInfo Extend(Type? definitionType, Type? applierType)
    {
        ...
        return new ConceptInfo(
            Name,
            definitionType is null ? DefinitionTypes : [.. DefinitionTypes, definitionType],
            DescriptionType,
            applierType is null ? ApplierTypes : [.. ApplierTypes, applierType]
        );
    }
}
